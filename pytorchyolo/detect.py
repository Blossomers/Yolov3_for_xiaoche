...
def detect(model, dataloader, output_path, img_size, conf_thres, nms_thres):
    ...
    for (img_paths, input_imgs) in tqdm.tqdm(dataloader, desc="Detecting"):
        ...
        # img_detections = torch.cat(tuple(img_detections), 0)
        # img_detections = torch.clamp(img_detections, 0.000, 5.000)
        # img_detections = list(img_detections)
...
def _draw_and_save_output_images(img_detections, imgs, img_size, output_path, classes):
    ...
    for (image_path, detections) in zip(imgs, img_detections):
        ...
        num += _draw_and_save_output_image(
...
def _draw_and_save_output_image(image_path, detections, img_size, output_path, classes):
    ...
    for x1, y1, x2, y2, conf, cls_pred in detections:
        ...
        # if int(cls_pred) == 0 or int(cls_pred) == 9 or int(cls_pred) == 1 or int(cls_pred) == 3:
        #     continue
...
def run():
    ...
    parser.add_argument("--img_size", type=int, default=2048, help="Size of each image dimension for yolo")
