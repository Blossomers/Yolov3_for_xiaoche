...
from yolo_models import load_model
...
from yolotest import _evaluate, _create_validation_data_loader
...
def run():
    ...
    parser.add_argument("-d", "--data", type=str, default="../config/custom.data",
                        help="Path to data config file (.data)")
    parser.add_argument("-e", "--epochs", type=int, default=302, help="Number of epochs")
    ...
    parser.add_argument("--logdir", type=str, default='./tf_dir',
    ...
    logger = Logger('./tf_dir')  # Tensorboard logger
    ...
    for epoch in range(args.epochs):
        ...
        for batch_i, (_, imgs, targets) in enumerate(tqdm.tqdm(dataloader, desc=f"Training Epoch {epoch}")):
            ...
            #             print(float(loss_components[0]))
            #             print(float(loss_components[1]))
            #             print(float(loss_components[2]))
        ...
        if epoch % args.checkpoint_interval == 1:
            checkpoint_path = f"./checkpoints/2_yolov3_ckpt_{epoch}.pth"
